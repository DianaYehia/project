...
if Menu=="Dashboard":
    option = st.selectbox(
    'Select the course preparation',
    ('completed','none'))
if Menu=="Dashboard":st.write('Test Preparation:', option)
if Menu=="Dashboard":df_test = df[df['test_preparation_course']==option]
if Menu=="Dashboard":df_gender= df_test[df['gender']]
if Menu=="Dashboard":df_math=df_test[df['math_score']]
if Menu=="Dashboard":df_reading=df_test[df['reading_score']]
if Menu=="Dashboard":df_writing=df_test[df['writing_score']]

if Menu=="Dashboard":fig4=px.pie(df_test, values=df_math, names=df_gender, title='Math scores vs gender',color_discrete_sequence=px.colors.sequential.Agsunset)
k1.plotly_chart(fig4, use_container_width=True)

if Menu=="Dashboard":fig7 = px.pie(df_test, values=df_reading, names=df_gender, title='Reading scores vs gender', color_discrete_sequence=px.colors.sequential.Agsunset)
k2.plotly_chart(fig7, use_container_width=True)

if Menu=="Dashboard":fig6 = px.pie(df_test, values=df_writing, names=df_gender, title='Writing scores vs gender', color_discrete_sequence=px.colors.sequential.Agsunset)
